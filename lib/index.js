    ...
    NanoSQLInstance.prototype.connect = function () {
        ...
        return new lie_ts_1.Promise(function (res, rej) {
            ...
            new utilities_1.CHAIN(_this._plugins.map(function (p) {
                ...
                var oldTable = _this.sTable;
                var completeConnect = function () {
                    _this.table(oldTable);
                ...
                _this.table("_util").query("select").where(["key", "=", "version"]).exec().then(function (rows) {
                    if (!rows.length) {
                        _this.table("_util").query("upsert", { key: "version", value: _this.version }).exec().then(function () {
