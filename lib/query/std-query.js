...
var _NanoSQLQuery = /** @class */ (function () {
            ...
            t.exec().then(function (json) {
                ...
                if (typeof _this._query.table === "string") {
                    ...
                    json.forEach(function (json) {
                        var maybeHeaders = Object.keys(json[0]);
    ...
    _NanoSQLQuery.prototype.exec = function () {
        ...
        if (["select", "upsert", "delete", "drop", "show tables", "describe"].indexOf(a) > -1) {
            ...
            if (a === "upsert") {
                // Do Row Filter
                if (this._db.rowFilters[this._query.table]) {
                    newArgs_1 = this._db.rowFilters[this._query.table](newArgs_1);
                }
                // Cast row types and remove columns that don't exist in the data model
                var inputArgs_1 = {};
                var models = this._db.dataModels[this._query.table];
                var k = 0;
                while (k < models.length) {
                    if (newArgs_1[models[k].key] !== undefined) {
                        inputArgs_1[models[k].key] = utilities_1.cast(models[k].type, newArgs_1[models[k].key]);
                    }
                    k++;
                }
                // insert wildcard columns
                if (this._db.skipPurge[this._query.table]) {
                    var modelColumns_1 = models.map(function (m) { return m.key; });
                    var columns = Object.keys(newArgs_1).filter(function (c) { return modelColumns_1.indexOf(c) === -1; }); // wildcard columns
                    columns.forEach(function (col) {
                        inputArgs_1[col] = newArgs_1[col];
                    });
                }
