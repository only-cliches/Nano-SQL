...
var _NanoSQLStorageQuery = /** @class */ (function () {
    ...
    _NanoSQLStorageQuery.prototype._tokenizer = function (column, value) {
        ...
        var words = (value || "").toLowerCase().replace(/[^\w\s]|_/gmi, "").replace(/\r?\n|\r|\t/gmi, "").replace(/\s+/g, " ").split(" ");
        // Step 2, stem away!
        if (args[1] === "english") {
            return words.map(function (w, i) { return ({ o: w, w: metaphone(stemmer(w)), i: i }); });
        }
        if (args[1] === "english-stem") {
            return words.map(function (w, i) { return ({ o: w, w: stemmer(w), i: i }); });
        }
        if (args[1] === "english-meta") {
            return words.map(function (w, i) { return ({ o: w, w: metaphone(w), i: i }); });
        }
        return words.map(function (w, i) { return ({ o: w, w: w, i: i }); });
    };
    _NanoSQLStorageQuery.prototype._clearFromSearchIndex = function (pk, complete) {
        var _this = this;
        ...
        var searchTable = "_" + table + "_search_";
        utilities_1.fastALL(columns, function (col, i, next) {
            ...
            _this._store.adapters[0].adapter.read(tokenTable + col, pk, function (row) {
                ...
                utilities_1.fastALL(Object.keys(wordsToRemove), function (word, j, done) {
                    _this._store.adapters[0].adapter.read(searchTable + col, word, function (wRow) {
                        if (!wRow) {
                            done();
                            return;
                        }
                        ...
                        _this._store.adapterWrite(searchTable + col, word, wRow, done);
                    });
                }).then(function () {
    ...
    _NanoSQLStorageQuery.prototype._updateSearchIndex = function (pk, newRowData, complete) {
        ...
        utilities_1.fastALL(columns, function (col, i, next) {
            ...
            _this._store.adapters[0].adapter.read(tokenTable + col, pk, function (row) {
                ...
                var newTokens = _this._tokenizer(col, newRowData[col]).map(function (o) { return ({ w: o.w, i: o.i }); });
                var oldTokenIdx = existing.tokens.map(function (t) { return t.i + "-" + t.w; });
                var newTokenIdx = newTokens.map(function (t) { return t.i + "-" + t.w; });
                var addTokens = newTokenIdx.filter(function (i) { return oldTokenIdx.indexOf(i) === -1; });
                var removeTokens = oldTokenIdx.filter(function (i) { return newTokenIdx.indexOf(i) === -1; });
                utilities_1.fastCHAIN([removeTokens, addTokens], function (tokens, j, nextTokens) {
                    ...
                    utilities_1.fastALL(Object.keys(reduceWords), function (word, k, nextWord) {
                        _this._store.adapters[0].adapter.read("_" + table + "_search_" + col, word, function (colRow) {
                            ...
                            switch (j) {
                                case 0: // remove
                                    searchIndex.rows.forEach(function (sRow, l) {
                                        if (sRow.id === pk) {
                            ...
                            _this._store.adapterWrite("_" + table + "_search_" + col, word, searchIndex, nextWord);
                        });
                    }).then(nextTokens);
                }).then(function () {
    ...
    _NanoSQLStorageQuery.prototype._upsert = function (next) {
        ...
        if (this._query.where) { // has where statement, select rows then modify them
            ...
            if (row_1[pk] !== undefined && this._query.comments.indexOf("_rebuild_search_index") === -1) {
            ...
            this._getRows(function (rows) {
                ...
                if (rows.length) {
                    utilities_1.fastALL(rows, function (r, i, done) {
                        _this._clearFromSearchIndex(r[_this._store.tableInfo[_this._query.table]._pk], function () {
            ...
            utilities_1.fastCHAIN(where, function (wArg, i, nextWArg) {
                ...
                _this._selectRowsByIndexOrSearch(wArg, function (rows) {
                    if (lastCommand_1 === "AND") {
                        ...
                        var i_1 = rows.length;
                        while (i_1--) {
                            idx_1[rows[i_1][_this.s.tableInfo[_this.q.table]._pk]] = true;
                        }
                        resultRows_1 = resultRows_1.filter(function (row) {
                            return idx_1[row[_this.s.tableInfo[_this.q.table]._pk]];
                        });
                    }
                    else {
                ...
                var pks = [];
                callback(resultRows_1.filter(function (r) {
    ...
    _RowSelection.prototype._selectRowsByIndexOrSearch = function (where, callback) {
        ...
        if (where[0].indexOf("search(") !== -1) {
            ...
            if (where[1].indexOf(">") !== -1) {
                whereType_1 = parseFloat(where[1].replace(">", ""));
            }
            if (where[1].indexOf("<") !== -1) {
                whereType_1 = parseFloat(where[1].replace("<", "")) * -1;
            }
            ...
            utilities_1.fastALL(columns, function (col, i, nextCol) {
                ...
                utilities_1.fastALL(searchTerms, function (term, j, nextTerm) {
                    var indexTable = "_" + _this.q.table + "_search_" + col;
                    ...
                    Object.keys(reducedResults).forEach(function (rowPK) {
                        ...
                        var locations = wordLocs.reduce(function (p, c) { return p + c.loc.length; }, 0);
                        if (!weights_1[rowPK]) {
                            weights_1[rowPK] = { weight: 0, locations: {} };
                        }
                        weights_1[rowPK].weight += (locations / docLength) + parseInt(args[0]);
                ...
                rowKeys.forEach(function (w) {
                    max = Math.max(max, weights_1[w].weight);
                });
                rowKeys.forEach(function (w) {
                    weights_1[w].weight = weights_1[w].weight / max;
                });
                utilities_1.fastALL(rowKeys.filter(function (pk) {
                    ...
                    callback(rows.map(function (r) { return (__assign({}, r, { _weight: weights_1[r[pk]].weight, _locations: weights_1[r[pk]].locations })); }));
                });
            });
            return;
        }
    ...
    _RowSelection.prototype._selectByRange = function (callback) {
        ...
        if (this.q.range) {
            ...
            if (r_1[0] > 0) { // positive limit value, we can send this straight to the adapter
                this.s._rangeRead(this.q.table, r_1[1], r_1[1] + r_1[0], false, callback);
            }
        ...
        var scanTable = function () {
            _this.s._read(_this.q.table, function (row, i, keep) {
                ...
                if (arraySearchCache.length) {
                    callback(rows.map(function (r) { return rowCache[r[PK]] ? rowCache[r[PK]] : r; }));
                }
                else {
                    callback(rows);
                }
    ...
    if (typeof where[0] !== "string") { // compound where statements
        ...
        return where.reduce(function (prev, wArg, idx) {
            ...
            if (wArg[0].indexOf("search(") !== -1 && searchCache) {
                compareResult = searchCache[idx].map(function (r) { return r[pk]; }).indexOf(singleRow[pk]) !== -1;
            }
            else {
                compareResult = _compare(wArg[2], wArg[1], utilities_1.objQuery(wArg[0], singleRow, ignoreFirstPath));
            }
        ...
        return _compare(where[2], where[1], utilities_1.objQuery(where[0], singleRow, ignoreFirstPath));
    }
};
/**
...
var _compare = function (val1, compare, val2) {
    ...
    switch (compare) {
        ...
        case "INTERSECT": return (columnValue || []).filter(function (l) { return (givenValue || []).indexOf(l) > -1; }).length > 0 ? true : false;
        // if array of values does not intersect with array column
        case "NOT INTERSECT": return (columnValue || []).filter(function (l) { return (givenValue || []).indexOf(l) > -1; }).length === 0 ? true : false;
