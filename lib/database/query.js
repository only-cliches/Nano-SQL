    ...
    _NanoSQLStorageQuery.prototype._updateORMRows = function (relation, fromPKs, add, primaryKey, complete) {
        ...
        this._store._read(relation._fromTable, fromPKs, function (rows) {
            utilities_1.fastALL(rows, function (row, i, rowDone) {
                ...
                _this._store._nsql.query("upsert", newRow).comment("ORM Update").manualExec({ table: relation._fromTable }).then(rowDone);
            }).then(complete);
        });
    };
...
var _RowSelection = /** @class */ (function () {
    ...
    _RowSelection.prototype._selectByKeys = function (callback) {
        ...
        if (this.q.where && typeof this.q.where[0] === "string") {
            ...
            var resultRows_1 = [];
            var lastCommand_1 = "";
            utilities_1.fastCHAIN(this.q.where, function (wArg, i, nextWArg) {
                if (wArg === "OR" || wArg === "AND") {
                    lastCommand_1 = wArg;
                    nextWArg();
                    return;
