...
var _MutateSelection = /** @class */ (function () {
     ...
     */
    _MutateSelection.prototype._maybeJoin = function (leftRow, onRow, complete, error) {
        var _this = this;
        var _a;
        ...
        var undefinedLeft = this.s.tableInfo[leftTable]._keys.reduce(function (prev, cur) { return prev[cur] = undefined, prev; }, {});
        var doJoin = function (rowData, joinIdx, joinDone) {
            ...
            var _combineRows = function (rData) {
            ...
            var rightPK = _this.s.tableInfo[join.table]._pk;
            var rightTable = join.table;
            var undefinedRight = _this.s.tableInfo[rightTable]._keys.reduce(function (prev, cur) { return prev[cur] = undefined, prev; }, {});
            var eachRow = function (row) {
                var _a;
                joinRowCount++;
                if (join.type === "right" || join.type === "outer") {
                    // keep track of which right side rows have been joined
                    rightHashes.push(utilities_1.deepGet(rightPK, row));
                }
            ...
            var rowsDone = function () {
                ...
                switch (join.type) {
                    case "left":
                        if (joinRowCount === 0) {
                            joinBuffer.newItem(__assign({}, rowData, (_a = {}, _a[rightTable] = undefinedRight, _a)));
                        }
                        joinBuffer.finished();
                        break;
                    case "inner":
                    ...
                    case "right":
                        if (joinRowCount === 0 && join.type === "outer") {
                            joinBuffer.newItem(__assign({}, rowData, (_b = {}, _b[rightTable] = undefinedRight, _b)));
                        }
                        // full table scan on right table :(
                        _this.s._rangeRead(join.table, undefined, undefined, true, function (rows) {
                            rows.forEach(function (row) {
                                var _a;
                                joinBuffer.newItem(__assign({}, rowData, (_a = {}, _a[leftTable] = undefinedLeft, _a[rightTable] = row, _a)));
                            });
                            joinBuffer.finished();
                        }, function (row) {
                            return rightHashes.indexOf(utilities_1.deepGet(rightPK, row)) === -1;
                        });
                        break;
                }
            };
            if (join.type === "cross") {
                ...
                var joinCondition = join.where || [];
                if (!joinCondition.length) {
                    error("No join condition!");
                    return;
                }
                // ["someTable.col", "=", "otherTable.id"]
                var rightTableOnLeftCondition = joinCondition[0].indexOf(join.table) === 0;
                var rightCol = rightTableOnLeftCondition ? joinCondition[0].split(".")[1] : joinCondition[2].split(".")[1];
                var rightValue_1 = rightTableOnLeftCondition ? utilities_1.deepGet(joinCondition[2], rowData) : utilities_1.deepGet(joinCondition[0], rowData);
                if (rightCol === rightPK) { // primary key (very fast)
                    _this.s.adapterRead(join.table, rightValue_1, function (row) {
                        eachRow(row);
                        rowsDone();
                    }, false);
                }
                else { // not primary key on right (full table scan)
                    var getRrightCol_1 = (rightTableOnLeftCondition ? joinCondition[0] : joinCondition[2]).split(".").filter(function (v, i) { return i > 0; }).join(".");
                    _this.s._rangeRead(join.table, undefined, undefined, true, function (rows) {
                        rows.forEach(eachRow);
                        rowsDone();
                    }, function (row) {
                        return utilities_1.deepGet(getRrightCol_1, row) === rightValue_1;
                    });
                }
            }
        };
        doJoin((_a = {}, _a[leftTable] = leftRow, _a), 0, complete);
    };
    /**
     * Peform a join command.
     *
    ...
    */
    ...
    _MutateSelection.prototype._executeQueryArguments = function (inputRows, callback, error) {
        ...
        if (this.q.join) {
            ...
            utilities_1.fastCHAIN(inputRows, function (row, i, next) {
                _this._maybeJoin(row, function (newRow) {
                    joinedRows_1.push(newRow);
                }, next, error);
            }).then(function () {
                inputRows = joinedRows_1;
                afterJoin();
            }).catch(error);
        }
        else {
