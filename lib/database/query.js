...
var _where = function (singleRow, where, rowIDX, ignoreFirstPath, searchCache, pk) {
    if (typeof where[0] !== "string") { // compound where statements
        var hasOr = where.indexOf("OR") !== -1;
        var decided = void 0;
        var conditions_1 = where.map(function (wArg, idx) {
            if (idx % 2 === 1)
                return wArg;
        ...
        var prevCondition = void 0;
        var skipSlots_1 = [];
        conditions_1.forEach(function (wArg, idx) {
            if (wArg === "AND" && skipSlots_1.indexOf(idx) === -1) {
                var stackAnds_1 = [];
                var stop_1 = false;
                conditions_1.forEach(function (wArg2, idx2) {
                    if (idx < idx2 && idx2 % 2 === 1 && !stop_1) {
                        if (wArg2 === "OR") {
                            stop_1 = true;
                        }
                        else {
                            stackAnds_1.push(idx2);
                        }
                    }
                });
                conditions_1[idx] = conditions_1[idx - 1] && conditions_1[idx + 1];
                stackAnds_1.forEach(function (stackIdx) {
                    conditions_1[idx] = conditions_1[idx] && conditions_1[stackIdx - 1] && conditions_1[stackIdx + 1];
                });
                conditions_1[idx - 1] = undefined;
                conditions_1[idx + 1] = undefined;
                stackAnds_1.forEach(function (stackIdx) {
                    conditions_1[stackIdx] = undefined;
                    conditions_1[stackIdx - 1] = undefined;
                    conditions_1[stackIdx + 1] = undefined;
                });
                skipSlots_1 = skipSlots_1.concat(stackAnds_1);
            }
        });
        return conditions_1.filter(function (c, i) { return c !== undefined && c !== "OR"; }).reduce(function (p, c) { return p || c; }, false);
    }
