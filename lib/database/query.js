...
var _NanoSQLStorageQuery = /** @class */ (function () {
     ...
     * @memberof _NanoSQLStorageQuery
     */
    _NanoSQLStorageQuery.prototype._getRows = function (onRows, onComplete, error) {
        var _this = this;
        if (this._isInstanceTable) {
            ...
            var i_1 = 0;
            var getRows_1 = function () {
                if (_this._query.batch) {
                    _this._query.limit = _this._query.batch;
                    _this._query.offset = i_1 * _this._query.batch;
                }
                new _RowSelection(_this, _this._query, _this._store, function (rows) {
                    var rF = rows.filter(function (r) { return r; });
                    onRows(rF, function () {
                        if (_this._query.batch) {
                            if (rF.length) {
                                i_1++;
                                utilities_1.setFast(getRows_1);
                            }
                            else {
                                onComplete();
                            }
    ...
    _NanoSQLStorageQuery.prototype._select = function (next, error) {
        ...
        this._getRows(function (rows, nextBatch) {
    ...
    _NanoSQLStorageQuery.prototype._upsert = function (next, error) {
        ...
        if (this._query.where) { // has where statement, select rows then modify them
            var newRows_1 = [];
            var num_1 = 0;
            this._getRows(function (rows, nextRows) {
                if (rows.length) {
                    ...
                    utilities_1.fastCHAIN(_this._query.actionArgs, function (inputData, k, nextRow, actionErr) {
                        ...
                        _this._syncORM("add", rows, newRows_1, function () {
                            _this._doAfterQuery(newRows_1, false, function () {
                                nextRows();
                            });
                        });
                    }).catch(error);
                ...
                if (rows.length) {
                    utilities_1.fastALL(rows, function (r, i, done) {
                        ...
                        _this._syncORM("del", rows, [], function () {
                            _this._doAfterQuery(rows, true, function () {
                                nextBatch();
                            });
                        });
                    });
