...
var _NanoSQLStorageQuery = /** @class */ (function () {
    ...
    _NanoSQLStorageQuery.prototype._tokenizer = function (column, value) {
        ...
        // Step 2, stem away!
        switch (args[1]) {
            case "english": return words.map(function (w, i) { return ({ o: w, w: metaphone(stemmer(w)), i: i }); });
            case "english-stem": return words.map(function (w, i) { return ({ o: w, w: stemmer(w), i: i }); });
            case "english-meta": return words.map(function (w, i) { return ({ o: w, w: metaphone(w), i: i }); });
        }
...
var _compare = function (where, wholeRow, isJoin) {
    if (whereLevenshtienCache[where[0]] === undefined) {
        whereLevenshtienCache[where[0]] = where[0].indexOf("levenshtein(") === 0 ? where[0].replace(/levenshtein\((.*,.*)\)/gmi, "$1").split(",").map(function (s) { return s.trim(); }) : [];
    }
    ...
    var getColValue = function () {
        if (whereLevenshtienCache[where[0]] && whereLevenshtienCache[where[0]].length) {
            var val = utilities_1.objQuery(whereLevenshtienCache[where[0]][1], wholeRow, isJoin);
            var key = (val || "") + "::" + whereLevenshtienCache[where[0]][0];
