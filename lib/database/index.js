            ...
            utilities_1.fastALL(Object.keys(tables), function (tableName, i, done) {
                ...
                var next = function () {
                    if (k < length) {
                        ...
                        if (row[pkKey]) {
                            _this._store.adapters[0].adapter.write(tableName, row[pkKey], row, function () {
                                k++;
    ...
    NanoSQLDefaultBackend.prototype.extend = function (next, args, result) {
        ...
        switch (args[0]) {
            ...
            case "idx":
                ...
                break;
            case "rebuild_search":
                var rebuildTables = (function () {
                    if (args[1])
                        return [args[1]];
                    return Object.keys(_this._store.tableInfo);
                })();
                utilities_1.fastALL(rebuildTables, function (table, i, done) {
