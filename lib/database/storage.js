...
var newQueryQ = function (_this) {
    return {
        ...
        currentID: {},
        running: false,
        add: function (doQeue, table, cb) {
            if (!doQeue) {
                cb(function (passArg, passCB) {
                    passCB(passArg);
                });
                return;
            }
            var id = _this._nsql.fastRand();
            _this.qq.cbs[id] = cb;
        ...
        loop: function () {
            ...
            setTimeout(function () {
                var totalRunning = 0;
                Object.keys(_this.qq.list).forEach(function (table) {
                    ...
                    if (_this.qq.list[table].length) {
                        if (_this.qq.currentID[table] !== _this.qq.list[table][0]) {
                            ...
                            _this.qq.cbs[_this.qq.list[table][0]](function (passArg, passCB) {
...
var _NanoSQLStorage = /** @class */ (function () {
     ...
     * @memberof _NanoSQLStorage
     */
    _NanoSQLStorage.prototype._secondaryIndexRead = function (table, condition, column, search, callback, queue) {
        var _this = this;
        this.qq.add(queue || false, "_" + table + "_idx_" + column, function (ready) {
            switch (condition) {
                case "=":
                    _this.adapters[0].adapter.read("_" + table + "_idx_" + column, _this._secondaryIndexKey(search), function (row) {
                        if (row !== undefined && row !== null) {
                            _this._read(table, (row["rows"] || []), function (rows) {
                                ready(rows, callback);
                            });
                        }
                        else {
                    ...
                    break;
                default:
                    _this.adapters[0].adapter.getIndex("_" + table + "_idx_" + column, false, function (index) {
     ...
     * @memberof _NanoSQLStorage
     */
    _NanoSQLStorage.prototype._write = function (table, pk, oldRow, newRow, complete, queue) {
        var _this = this;
        this.qq.add(queue || false, table, function (ready) {
            if (!oldRow) { // new row
                _this.adapterWrite(table, pk, newRow, function (row) {
                ...
                var setRow_1 = __assign({}, oldRow, newRow, (_a = {}, _a[_this.tableInfo[table]._pk] = pk, _a));
                var sameKeys_1 = Object.keys(setRow_1).filter(function (key) {
                    return setRow_1[key] === oldRow[key];
                });
                if (_this.tableInfo[table]._secondaryIndexes.length) {
                    _this._clearSecondaryIndexes(table, pk, oldRow, sameKeys_1, function () {
                        _this._setSecondaryIndexes(table, pk, setRow_1, sameKeys_1, function () {
                            _this.adapterWrite(table, pk, setRow_1, function (row) {
                                ready(row, complete);
                            });
                        });
                    });
    ...
    _NanoSQLStorage.prototype.adapterRead = function (table, pk, complete, queue) {
        var _this = this;
        this.qq.add(queue || false, table, function (ready) {
            _this.adapters[0].adapter.read(table, pk, function (row) {
                ready(row, complete);
            });
        });
    };
    _NanoSQLStorage.prototype.adapterWrite = function (table, pk, data, complete, error, queue) {
        var _this = this;
        this.qq.add(queue || false, table, function (ready) {
            var result;
            utilities_1.fastALL(_this.adapters, function (a, i, done) {
                if (a.waitForWrites) {
