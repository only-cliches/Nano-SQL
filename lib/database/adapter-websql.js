...
var _WebSQLStore = /** @class */ (function () {
    ...
    _WebSQLStore.prototype.batchRead = function (table, pks, callback) {
        this._sql(false, "SELECT data from " + this._chkTable(table) + " WHERE id IN (" + pks.map(function (p) { return "?"; }).join(", ") + ") ORDER BY id", pks.map(function (k) { return typeof k === "string" ? "'" + k + "'" : k; }), function (result) {
    ...
    _WebSQLStore.prototype.rangeRead = function (table, rowCallback, complete, from, to, usePK) {
        ...
        stmnt += " ORDER BY id";
        this._sql(false, stmnt, getKeys.map(function (k) { return typeof k === "string" ? "'" + k + "'" : k; }), function (result) {
            var i = 0;
            var getRow = function () {
