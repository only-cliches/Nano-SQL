...
export class _NanoSQL_Storage {
    ...
    public _rebuildTries(callBack: Function) {
        ...
        Object.keys(t._tables).forEach((tableID) => {
            ...
            if (tableName.indexOf("_") !== 0) { // only check non internal tables
     ...
     * @memberOf _NanoSQL_Storage
     */
    public _delete(tableName: string, rowID: string|number, callBack?: (success: boolean) => void, transactionID?: number): void {
        let t = this;
     ...
     * @memberOf _NanoSQL_Storage
     */
    public _readArray(tableName: string, pkArray: any[], callBack: (rows: DBRow[]) => void ) {
        let rows: any[] = [];
        let ptr = 0;
        const readRow = () => {
    ...
    public _read(tableName: string, row: string|number|Function, callBack: (rows: any[]) => void, readIndex?: boolean): void {
        ...
        if (t._storeMemory) {
            ...
            if (row === "all" || typeof row === "function") {
                ...
                if (row === "all") {
                    t._indexRead(tableName, allRows.filter((r) => r) as DBRow[], callBack, readIndex);
                } else {
                    t._indexRead(tableName, allRows.filter((r) => row(r)) as DBRow[], callBack, readIndex);
                }
            } else {
                t._indexRead(tableName, [rows[row]].filter((r) => r) as DBRow[], callBack, readIndex);
            }
            return;
        }
