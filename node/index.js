...
var orm_query_1 = require("./query/orm-query");
...
var NanoSQLInstance = (function () {
    ...
    NanoSQLInstance.prototype.connect = function () {
        ...
        return new lie_ts_1.Promise(function (res, rej) {
            ...
            new utilities_1.CHAIN(_this._plugins.map(function (p) {
                ...
                t._tableNames.forEach(function (table) {
                    var i = _this._models[table].length;
                    _this.relations[table] = [];
                    _this.relationColumns[table] = [];
                    var _loop_1 = function () {
                        var p = _this._models[table][i];
                        if (p.props && _this._tableNames.indexOf(p.type.replace("[]", "")) !== -1) {
                            ...
                            _this.relationColumns[table].push(p.key);
                            _this.relations[table].push({
                                column: p.key,
                                toTable: p.type.replace("[]", ""),
                                toColumn: mapTo_1,
                                type: p.type.indexOf("[]") === -1 ? "single" : "array",
                                toType: mapTo_1 ? Object.keys(_this._models).reduce(function (prev, cur) {
                                    if (cur === p.type.replace("[]", "")) {
                                        return _this._models[cur].reduce(function (p, c) {
                                            if (c.key === mapTo_1) {
                                                return c.type.indexOf("[]") !== -1 ? "array" : "single";
                                            }
                                            return p;
                                        }, "");
                                    }
    ...
    NanoSQLInstance.prototype.updateORM = function (action, column, relationIDs) {
        if (Array.isArray(this.sTable))
            return null;
        return new orm_query_1._NanoSQLORMQuery(this, this.sTable, action, column, relationIDs);
    };
