...
var _NanoSQLStorageQuery = (function () {
    ...
    _NanoSQLStorageQuery.prototype._select = function (next) {
        ...
        this._getRows(function (rows) {
            var canCache = !_this._query.join && !_this._query.orm && _this._store._doCache && !Array.isArray(_this._query.table);
            if (!["having", "orderBy", "offset", "limit", "actionArgs", "groupBy", "orm", "join"].filter(function (k) { return _this._query[k]; }).length) {
                if (canCache)
                    _this._store._cache[_this._query.table][queryHash] = rows;
                ...
                new _MutateSelection(_this._query, _this._store)._executeQueryArguments(rows, function (resultRows) {
                    if (canCache)
                        _this._store._cache[_this._query.table][queryHash] = resultRows;
    ...
    _NanoSQLStorageQuery.prototype._upsert = function (next) {
        ...
        if (this._query.where) {
                ...
                if (rows.length) {
                    new utilities_1.CHAIN(rows.map(function (r) {
                        return function (rowDone) {
                            _this._store._write(_this._query.table, r[pk], r, _this._query.actionArgs || null, rowDone);
                        };
                    })).then(function (rows) {
                        var result = [].concat.apply([], rows);
                        _this._store._cache[_this._query.table] = {};
                        _this._query.result = [{ msg: rows.length + " row(s) modfied.", affectedRowPKS: result.map(function (r) { return r[pk]; }), affectedRows: result }];
                        next(_this._query);
                    });
                }
                else {
            ...
            var row = this._query.actionArgs || {};
            this._store._cache[this._query.table] = {};
            this._store._write(this._query.table, row[pk], null, row, function (result) {
            ...
            this._getRows(function (rows) {
                ...
                if (rows.length) {
                    new utilities_1.ALL(rows.map(function (r) {
                        ...
                        _this._store._cache[_this._query.table] = {};
                        _this._query.result = [{ msg: rows.length + " row(s) deleted.", affectedRowPKS: rows.map(function (r) { return r[_this._store.tableInfo[_this._query.table]._pk]; }), affectedRows: rows }];
                        next(_this._query);
                    });
                }
                else {
    ...
    _NanoSQLStorageQuery.prototype._drop = function (next) {
        ...
        if (this._query.transaction) {
            ...
            this._store._rangeRead(this._query.table, undefined, undefined, doDrop);
        }
    };
            ...
            return function (rowResult) {
                new utilities_1.ALL(ormQueries.map(function (orm) {
                    return function (ormResult) {
                        ...
                        var isArray = false;
                        var cacheKey = _this.q.table + "-" + orm.key;
                        var otherTable = "";
                        if (_this._ormTableCache[cacheKey]) {
                            otherTable = _this._ormTableCache[cacheKey][0];
                            isArray = _this._ormTableCache[cacheKey][1];
                        }
                        else {
                            otherTable = _this.s.models[_this.q.table].reduce(function (prev, cur) {
                            ...
                            _this._ormTableCache[cacheKey] = [otherTable, isArray];
                        }
                        if (otherTable) {
                            var otherPK_1 = _this.s.tableInfo[otherTable]._pk;
                            var ormWhere = [otherPK_1, (isArray ? "IN" : "="), row[orm.key]];
                            var q = _this.s._nsql.table(otherTable)
                                .query("select", orm.select);
                            if (orm.where) {
                                if (Array.isArray(orm.where)) {
                                    q.where(typeof orm.where[0] === "string" ? [ormWhere, "AND", orm.where] : orm.where.concat(["AND", ormWhere]));
                                }
                                else {
                                    q.where(function (r, idx) {
                                        return (isArray ? row[orm.key].indexOf(r[otherPK_1]) !== -1 : row[orm.key] === r[otherPK_1]) && orm.where(r, idx);
                                    });
                                }
                            }
                            else {
                            ...
                            q.exec().then(function (rows) {
                                if (!rows.filter(function (r) { return r; }).length) {
                                    ...
                                    row[orm.key] = isArray ? rows.filter(function (r) { return r; }) : rows[0];
                                }
                                ormResult();
                            });
                        }
...
var _RowSelection = (function () {
    ...
    _RowSelection.prototype._selectRowsByIndex = function (where, callback) {
        ...
        if (where[1] === "BETWEEN") {
            ...
            if (secondaryIndexKey) {
                ...
                new utilities_1.ALL(where[2].map(function (i) {
                    return function (done) {
                        _this.s._adapter.indexOfPK(idxTable_1, i, function (idx) {
                            done(idx);
                        });
                    };
                })).then(function (ranges) {
                    _this.s._rangeRead(idxTable_1, ranges[0], ranges[1], function (rows) {
                ...
                new utilities_1.ALL(where[2].map(function (i) {
                    return function (done) {
                        _this.s._adapter.indexOfPK(_this.q.table, i, function (idx) {
                            done(idx);
                        });
                    };
                })).then(function (ranges) {
                    _this.s._rangeRead(_this.q.table, ranges[0], ranges[1], function (rows) {
                        callback(rows);
                    });
                });
            }
    ...
    _RowSelection.prototype._selectByRange = function (callback) {
        ...
        if (this.q.range) {
            ...
            this.s._adapter.getIndex(this.q.table, false, function (pks) {
                var fromIdx = r_1[0] > 0 ? r_1[1] : pks.length + r_1[0] - r_1[1];
                var from = pks[fromIdx];
                if (!from) {
                    callback([]);
                    return;
                }
                if (Math.abs(r_1[0]) === 1) {
                    _this.s._read(_this.q.table, [from], callback);
                }
                else {
                    var toIdx = fromIdx;
                    ...
                    var to = pks[toIdx];
                    _this.s._rangeRead(_this.q.table, fromIdx, toIdx, callback);
                }
            });
        }
        else {
