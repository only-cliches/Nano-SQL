...
/***/ function(module, exports, __webpack_require__) {
	...
	var typescript_map_1 = __webpack_require__(1);
	var typescript_promise_1 = __webpack_require__(2);
	var Some_SQL_Memory_1 = __webpack_require__(3);
	var someSQL_Instance = (function () {
	    ...
	    someSQL_Instance.prototype.on = function (actions, callBack) {
	        ...
	        actions.split(' ').forEach(function (a) {
	            ...
	            _this._callbacks.get(_this._selectedTable).get(a).push(callBack);
	        });
	        return this;
	    };
	    ...
	    someSQL_Instance.prototype.doAction = function (actionName, actionArgs) {
	        var t = this;
	        var l = t._selectedTable;
	        var a = t._actions.get(l)[actionName];
	        ...
	        return new someSQL_Promise(this, function (res, rej) {
	            t._backend.exec(_t, t._query, function (rows) {
	                t._triggerEvents.forEach(function (e) {
	                    t._callbacks.get(_t).get(e).concat(t._callbacks.get('*').get(e)).forEach(function (cb) {
	                        cb.apply(t, [{
	                                type: e,
	                                table: _t,
	                                query: t._query,
...
/***/ function(module, exports, __webpack_require__) {
	...
	"use strict";
	var some_sql_1 = __webpack_require__(4);
	var typescript_promise_1 = __webpack_require__(2);
	var typescript_map_1 = __webpack_require__(1);
	var someSQL_MemDB = (function () {
	    ...
	    someSQL_MemDB.prototype.connect = function (models, callback) {
	        for (var table in models) {
	            this._newModel(table, models.get(table));
	        }
	        callback();
	    };
	                ...
	                if (hasWhere.length) {
	                    ...
	                    rows_1.forEach(function (v, k) {
	                        for (var key in t._act.get('args')) {
	                            ...
	                            msg_1.push(JSON.parse(JSON.stringify(t._act.get('args')[key])));
	                        }
	                    ...
	                    t._models.get(t._sT).forEach(function (m) {
	                        if (m['type'] == 'uuid' && !t._act.get('args')[m['key']]) {
	                            t._act.get('args')[m['key']] = some_sql_1.someSQL_Instance.uuid();
	                        }
	                    ...
	                    var i_1 = t._act.get('args')[key_1];
	                    if (t._tIndex.get(t._sT).indexOf(i_1) == -1) {
	                        t._tIndex.get(t._sT).push(i_1);
	                    }
	                    else {
	                        t._tCacheI.forEach(function (v, k) {
	                            if (v && v.indexOf(i_1) != -1) {
	                                t._tCacheI.delete(k);
	                                t._immu.delete(k);
	                            }
	                        });
	                    }
	                    t._tables.get(t._sT)[i_1] = t._act.get('args');
	                    msg_1 = JSON.parse(JSON.stringify(t._act.get('args')));
	                }
	                callBack(msg_1);
	                break;
	            case "select":
	                if (!t._immu.get(t._cacheKey)) {
	    ...
	    someSQL_MemDB.prototype._where = function (tableArray) {
	        ...
	        return tableArray.filter(function (v, k) {
	            ...
	            t._mod.map(function (mo) {
	                if (mo['type'] == 'orWhere') {
	                    mo.get('args').forEach(function (a) {
	                        ors.push(a);
	                    });
	                }
	            });
