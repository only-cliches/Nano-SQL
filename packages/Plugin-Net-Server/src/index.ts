...
export class nanoSQLServer {
    ...
    public connect(serverArgs: {
        ...
        secretKey: string; // should be identical for all servers & clusters
        multiCluster?: {
        ...
        offlineSyncRequest?: (database: string, table: string, session: InanoSQLSession) => Promise<boolean | ((rowData: any) => boolean)>;
